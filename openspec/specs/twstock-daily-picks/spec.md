...
L3 每日選股推播場景 twstock-notifying-dailypicks 的行為契約（v2.0-alpha）：對 watchlist 做
Python 前濾（可算訊號縮成 shortlist）、委派 twstock-module 取數、Claude 本體判讀 shortlist、
委派 twstock-notify 推播 LINE。排程外置（不內建 cron）、stateless；推播附免責與 data_gaps 揭露。
全市場掃描/快取/Dashboard/歷史 diff 為 v2.0-beta/future。
