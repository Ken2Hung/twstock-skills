...
3. **目標價**：數值（新台幣元），SHALL 為扣除交易成本後的淨值基準
4. **止損價**：數值（新台幣元），SHALL 為扣除交易成本後的淨值基準
5. **核心理由**：條列，MUST 為 1 至 3 點
6. **風險提示**：非空文字
7. **data_gaps**：陣列，忠實反映模組回傳之資料缺口，MUST NOT 省略或偽裝為空

綜合評分 SHALL 依基本面 30% × 技術面 30% × 籌碼面 40% 的固定佔位權重加權；本 change SHALL NOT 定義三面向下的個別指標細節（留待後續 change）。任一欄位缺漏、信號超出列舉集合、評分超出 0-100、或核心理由不在 1 至 3 點範圍內，該輸出 MUST 視為不合規。
...
#### Scenario: 評分採固定佔位權重且不展開指標細節

- **GIVEN** 場景計算某標的的綜合評分
- **WHEN** 加權基本面、技術面、籌碼面三個面向
- **THEN** 綜合評分 MUST 採基本面 30%、技術面 30%、籌碼面 40% 的固定佔位權重
- **AND** 本 change SHALL NOT 規定各面向底下的個別指標定義

### Requirement: 目標價與止損價引用交易成本真相來源

目標價與止損價的計算 MUST 引用 tw-market-reference（twstock-screening-stocks/references/tw-market-rules.md）所定義之台股交易成本作為單一真相來源：買進 0.1425%（手續費）、賣出 0.4425%（手續費 0.1425% + 證交稅 0.3%）。場景層 SHALL NOT 於場景層硬編碼另一套交易成本數字。價位的實際計算方式（淨值基準、方向性語意等）留待 Change 4 定義；本 change 只規範「須引用單一成本來源、禁止硬編碼」。標的若觸及漲跌停（±10%）或位於 TWSE 處置名單，場景 MUST 依 tw-market-reference 於風險提示欄位標註流動性風險。

#### Scenario: 目標價止損價引用單一成本來源

- **GIVEN** 場景計算某標的的目標價與止損價
- **WHEN** 產出這兩個價位欄位
- **THEN** 計算 MUST 引用 tw-market-reference 的買進 0.1425%、賣出 0.4425% 成本
- **AND** 場景 SHALL NOT 於場景層硬編碼另一套交易成本數字
