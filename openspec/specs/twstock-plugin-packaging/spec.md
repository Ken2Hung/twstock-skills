# twstock-plugin-packaging

## Purpose

專案以 Claude Code plugin 形式可安裝：.claude-plugin/ manifest（plugin.json + marketplace.json）
+ 6 支 skill 位於 skills/。跨 skill 相對 import 於 plugin 佈局下仍解析（同層兄弟）；plugin
（/plugin install）與手動（cp -R skills/twstock-* ~/.claude/skills/）兩種安裝法並存。
