...
籌碼面 40% SHALL 內部配置為：三大法人買賣超連續性 60%、融資融券變化 40%（股權分散趨勢列 v-next，不在 v1.0）。法人子項 SHALL 僅計外資（`Foreign_Investor`）與投信（`Investment_Trust`）之淨買超，SHALL NOT 計入自營避險（`Dealer_Hedging`、`Foreign_Dealer_Self`）。法人淨買超與融資／融券變化 MUST 以**相對量**（佔同期成交量比）計，SHALL NOT 以絕對張數或金額計，以免評分系統性偏向大型股。融資「大增」MUST 以量化門檻定義（5 日融資增量佔同期成交量比超過 `scoring-model.md` 所定門檻）。籌碼面回看窗口 SHALL 統一為 5 日。`scoring-model.md` MUST 為每個籌碼指標定義方向語意（正向／負向）、回看窗口與量化門檻，使評分可驗證。
...
- **THEN** 每個籌碼指標 MUST 標明方向語意（正向／負向）與回看窗口，使 scenario 可驗證
